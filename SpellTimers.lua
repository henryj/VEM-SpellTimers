-- **********************************************************
-- **             Voice Encounter Mods - SpellsUsed            **
...
VEM_SpellTimers_Settings = {}
local settings = default_settings

local L = VEM_SpellsUsed_Translations
...
do 
	local function creategui()
		...
		local CurCount = 0
		local panel = VEM_GUI:CreateNewPanel(L.TabCategory_SpellsUsed, "option")
		...
		do
			...
			resetbttn:SetScript("OnClick", function(self)
				table.wipe(VEM_SpellTimers_Settings)
		...
		do
				...
				return function(self)
					...
					if field == "bartext" and settings.spells[self.guikey].spell and settings.spells[self.guikey].spell > 0 then
						...
						if spellinfo == nil then
							VEM:AddMsg("Illegal SpellID found. Please remove the Spell "..settings.spells[self.guikey].spell.." from your VEM Options GUI (spelltimers)");
						else
			...
			function createnewentry()
				...
				if VEM_GUI_OptionsFramePanelContainer.displayedFrame and CurCount > 1 then
					VEM_GUI_OptionsFrame:DisplayFrame(panel.frame)
				end
	...
	VEM:RegisterOnGuiLoadCallback(creategui, 19)
end
...
do
	...
	local mainframe = CreateFrame("frame", "VEM_SpellTimers", UIParent)
	local spellEvents = {
	...
	mainframe:SetScript("OnEvent", function(self, event, ...)
		if event == "ADDON_LOADED" and select(1, ...) == "VEM-SpellTimers" then
			...
			settings = VEM_SpellTimers_Settings
			addDefaultOptions(settings, default_settings)
			...
			if settings.own_bargroup then
				...
				addDefaultOptions(SpellBars.options, VEM.Bars.options)
			else
				SpellBars = VEM.Bars
			end --]]
			SpellBars = VEM.Bars
		...
		elseif settings.enabled and event == "COMBAT_LOG_EVENT_UNFILTERED" and spellEvents[select(2, ...)] then
			...
			if settings.only_from_raid and VEM:GetRaidUnitId(fromplayer) == "none" then return end

			guikey = SpellIDIndex[spellid]
			v = (guikey and settings.spells[guikey])
			if v and v.enabled == true then
				if v.spell ~= spellid then
					print("VEM-SpellTimers Index mismatch error! "..guikey.." "..spellid)
				end
